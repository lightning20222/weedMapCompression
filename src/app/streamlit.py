...
if not image_files or not mask_files:
    ...
    # Each drone scans a different area - assign different masks to different drones
    all_mask_paths = [os.path.join(MASK_DIR, f) for f in mask_files]
    
    # Cycle through available masks if we have more drones than masks
    drone_masks = [all_mask_paths[i % len(all_mask_paths)] for i in range(num_drones)]
    drone_heatmaps = simulate_drone(drone_masks, drop_prob=drop_prob)
    
    st.info(f"Drones scanning different field sections: {num_drones} drones using masks from the dataset")
